...
def main_menu(user_id: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="🚀 Advanced/Premium", callback_data="menu_advanced"),
            InlineKeyboardButton(text="⚙️ Settings", callback_data="menu_settings"),
        ],
        [InlineKeyboardButton(text="📚 Glossary | Глоссарий для новичков\nдля помощи в навигации", callback_data="glossary")]
    ])
...
def advanced_menu(user_id: int):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text=lock("✍️ Fix sentence", "correction"), callback_data="mode_correction"),
        ],
        [
...
def level_kb(current: str):
    rows = [
        ...
        [InlineKeyboardButton(text="⬅️ Back to menu | Назад в меню", callback_data="back_menu")],
    ]
...
async def main():
    ...
    async def show_help(message: Message):
        ...
        "📚 Learning — обучение\n"
        "Explain - Объяснение любой темы\n"
        "Summary - Небольшой конспект и примерами\n"
        "🧠 Practice — практика\n"
        "Quiz - несколько вопросов по интересующей теме\n"
        "Practice"
    ...
    async def change_level_action(message: Message):
        current = get_level(message.from_user.id)
        await message.answer(
            f"🎯Current level: {current}\nChoose your level/Выберите ваш уровень:" + "\n\n" + get_phrase(),
    ...
    async def change_level_cmd(message: Message):
        await change_level_action(message)

    @dp.message(Command("glossary"))
    async def glossary_cmd(message: Message):
        await message.answer(format_glossary())
    ...
    async def cb(call: CallbackQuery, state: FSMContext):
        ...
        if data == "help":
            ...
            await change_level_action(call.message)
            ...
            await call.message.edit_text(
                format_glossary(),
                reply_markup=settings_menu(call.from_user.id)
            )
            ...
            USER_LEVEL[call.from_user.id] = level
            await call.message.edit_text(f"✅ Level set to | Уровень установлен на: {level}" + "\n\n" + get_phrase(), reply_markup=main_menu(call.from_user.id))
